"""Word V7 — Notoriedad Gama 2026
Tras reunión Cora + Owner 18/05 + plan V7 aprobado.
Bifurcación sistemática Total + C+/C + Pref-Gama.
Reusa template V6 + capítulos nuevos posicionamiento/DNA + perfil recordadores.
Contiene placeholder visible [DIF-9-1: Rio migrado] pendiente confirmación Cora.
"""
import sys
...
OUTPUT = r"C:\rAUL\03-projects\consultoria-externa\gama-notoriedad-2026\03-deck\V7\2026-05-18_Notoriedad-Gama-2026_V7.docx"
...
def build_portada(doc):
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("\nVersión V7  ·  Bifurcación Total + C+/C + Pref-Gama")
    ...
    metadata = [
        ...
        ("Status", "V7 FINAL — ajustes Cora email 18/05 17:04 incorporados"),
    ]
...
def build_executive(doc):
    ...
    add_callout_box(doc,
        "Gama tiene el DNA experiencial más fuerte y consolidado del mercado: ahora sobreíndice en 6 atributos "
        "(vs 4 en V3), incluyendo Atención (z=+0.52) y Rapidez (+0.62). El segmento natural (C+/C) ve a la marca "
        "incluso mejor que el Total en los atributos experienciales. Pero Rio cambió de modelo mental — ahora compite "
        "en precio (68% de sus preferentes citan precio), no en experiencia. Esto deja a Gama sin competidor "
        "experiencial directo de peso, lo que abre una oportunidad de comunicación más asertiva del territorio relacional.")
    ...
    add_metodologia_box(doc,
        "Resumen ejecutivo sintetiza hallazgos cuantitativos de CU-1 a CU-9 (n=402, m.e. ±4.89% al 95%). Las cifras de "
        "Pref-Gama (n=32) son referenciales. Las cifras de Gama por categoría son referenciales (n<30 en casi todos los "
        "casos). El análisis cualitativo del 2026 (corpus App Gama + Gama Club) NO se incluye como conclusión (era de "
        "otro estudio); solo se usan verbatims para confirmar posicionamiento cuantitativo, según decisión Cora 18/05.")


def build_marco(doc):
    add_h1(doc, "2. Marco del documento V7")

    add_h3(doc, "2.1 Qué es V7 (y qué cambia vs V6)")
    add_para(doc, "V7 incorpora 4 cambios estructurales respecto a V6 (entregado el 2026-05-18 1:30 AM), todos "
                  "derivados de la reunión Cora + Owner del 18/05 9:38 AM:")
    add_bullet(doc, "Bifurcación sistemática Total + C+/C + Pref-Gama en CADA capítulo sustantivo (no solo Bloque 1).")
    add_bullet(doc, "Capítulo nuevo de Posicionamiento + Mundo de marca + DNA competitivo (recupera análisis V3 perdido).")
    add_bullet(doc, "Capítulo nuevo de perfil del recordador publicitario + re-segmentación de todas las preguntas según recall.")
    add_bullet(doc, "Pirámide Minto explícita por bloque (lámina DATOS → lámina ANÁLISIS) para que el cliente vea los números antes de la interpretación.")
...
def build_decisiones_pendientes(doc):
    ...
    add_h3(doc, "Decisiones aplicadas en este V7 final")
    add_bullet(doc, "Rio migrado a precio-dominante (confirmado por Cora email 18/05 16:18) — actualizado en Cap 1.3 "
                    "con nota sobre apertura de tiendas como factor compatible.")
    add_bullet(doc, "Plan Suárez como 2° en preferencia C+/C (15.4%) — destacado en Cap 1.2 y Cap 1.3 según "
                    "observación explícita de Cora (email 18/05 17:04).")
    add_bullet(doc, "Reformulación DNA — \"Gama outlier estructural sostenido\" matizado con que esto NO es "
                    "excluyente con abordar precio para captar no-clientes (input Cora email 17:04).")
    add_bullet(doc, "Cap 3 enriquecido con contexto del modelo mental (7 cadenas precio-dominantes vs Gama outlier) "
                    "como insumo para análisis de precios por categoría.")
    add_bullet(doc, "Cap 5.6 nuevo — vías creativas para abordar precio sin disminuir percepción de valor "
                    "(promociones diferenciadas, coleccionables, mensaje del anzuelo, lealtad visible).")
    add_bullet(doc, "Cap 5.7 nuevo — prerrequisito metodológico de análisis de sensibilidad antes de cualquier "
                    "ajuste operativo de precio.")
    add_bullet(doc, "Caveats Pref-Gama (n=32) visibles en cada cifra individual + tabla resumen.")
    add_bullet(doc, "Categorías P30/P32 con porcentajes visibles + flag REFERENCIAL + lectura cualitativa.")

    add_h3(doc, "Entregables que acompañan a este Word V7")
    add_bullet(doc, "Documento metodológico separado (.docx) — explicación de análisis estadísticos para Cora + cliente.")
    add_bullet(doc, "HTML interactivo simple con data bruta + variables de cruce (NSE, género, edad, marca preferida, municipio).")
    add_bullet(doc, "Plan V7 (.docx) — ya dropeado el 18/05 11:21 AM como referencia del scope acordado.")

    add_h3(doc, "Próximo paso — PPTX V7")
    add_para(doc, "Una vez aprobado este Word V7 por Owner y Cora, Vivienne renderea PPTX en 3 invocaciones "
                  "separadas (charts standalone / deck assembly / RE assembly) con paleta Gama corregida (rojo más "
                  "saturado vs vino tinto del V6), aplicando Pirámide Minto explícita en cada bloque (lámina "
                  "descriptiva → lámina analítica) y estilo infografía vs bullet-point per pedido Cora reunión 18/05.")


def main():
    print("Iniciando construccion del Word V7...")
    doc = Document()
    ...
    print("  Executive summary..."); build_executive(doc)
    print("  Marco V7..."); build_marco(doc)
    print("  Ficha tecnica..."); build_ficha(doc)
    print("  Capitulo 1 - Embudo + drivers..."); build_cap1(doc)
    print("  Capitulo 2 - Posicionamiento + DNA..."); build_cap2(doc)
    print("  Capitulo 3 - Precios + categorias..."); build_cap3(doc)
    print("  Capitulo 4 - Comunicacion PTL+DTLS..."); build_cap4(doc)
    print("  Capitulo 5 - Sintesis bifurcada..."); build_cap5(doc)
    print("  Anexos A/B/C..."); build_anexos(doc)
    print("  Decisiones pendientes..."); build_decisiones_pendientes(doc)
