...
- **Sección 3:** Detalle por nodo. Cada entrada incluye: mecanismo técnico con parámetros numéricos, productos donde vive, RTBs verificables con referencia documental, estado de diferenciación competitiva, observaciones legales aplicables, y documentos internos que el abogado puede pedir.
...
| Nodo | Título funcional | Anchor / Candidato 1 | Línea comercial primaria | Estado competitivo | Observación legal |
|---|---|---|---|---|---|
...
- Decisión interna sobre alcance: función asumida presente en toda la gama Exceline / Exceline Profesional / Genius con función de temporizado post-falla
...
#### Observaciones legales aplicables
...
- GSM-NG: 150 ms
- Decisión interna sobre alcance: gama completa Exceline / Exceline Profesional / Genius
...
Existen candidatos alternativos preparados internamente (contingencia OMPI) con raíces Flicker+Trap, Flicker+Block, Flicker+Shield, PulseGuard, SagGuard — disponibles si la búsqueda formal confirma colisión activa.
...
GOCT (Clase 10 fija: curva fría 10, caliente 3); GUCT+ (Clase ajustable 5-30); GSPT / GSPT-MV (Clase 10 fija, cold/hot documentado con curva gráfica en HDE); GIII+ / GIII+MV (Clase ajustable 5-30, cold/hot documentado); GOC (Clase 10 fija, cold curve 10, hot curve 3). Decisión interna sobre alcance: curva inversa I-t cold/hot asumida universal en gama Exceline / Exceline Profesional / Genius.
...
Diferenciador real: no hay diferenciador genuinamente único en la función base con los RTBs disponibles. La decisión interna sobre alcance asume la función universal. Si I+D puede documentar un parámetro de implementación que exceda el estándar IEC (factor de enfriamiento diferencial propio, parámetro de memoria térmica con valor específico no derivado de la norma), ese RTB cambiaría el estado a amarillo o verde.
...
- El GIII+MV es el portador principal del nombre ForensLog según postura del deck (Escenario A aprobado internamente)
...
Nota al abogado: "Log" tiene presencia en software/apps en Clase 9 amplia. La búsqueda fonética debe incluir la subclase de software de diagnóstico industrial. Existen candidatos alternativos internos (ForensTrace, FaultVault, DiagLog) disponibles si la búsqueda encuentra colisión activa.
...
Existen candidatos alternativos internos con raíces Gate, Hold, Block, Halt (contingencia sectorial) disponibles si la búsqueda confirma colisión activa.
...
Postura del comité IP: TaskMemory es el candidato principal para SAPI VE. Existen candidatos alternativos internos específicamente para IMPI MX si la búsqueda confirma colisión (ModeKeep, StateHold, CycleResume, RetainMode).
...
Los siguientes documentos son necesarios para que el abogado pueda validar los RTBs declarados en este Anexo. La lista consolida los gaps identificados por el inventario técnico interno y los requerimientos adicionales de la evaluación legal interna.
...
- **Urgencia:** baja. Puede resolverse en cualquier momento antes de la presentación; no afecta la registrabilidad del nodo ni la observación legal del abogado.
...
El inventario técnico documenta la discrepancia: GIII+MV = 100 últimas fallas; GIII+ estándar = 20 últimas fallas; GSPT-MV = 80 últimas fallas. El anchor ForensLog del proyecto previo referenciaba "100-Fault Forensic History" que corresponde específicamente al GIII+MV. El análisis competitivo confirma el hallazgo con fuentes primarias. La evaluación interna resuelve la discrepancia: postura del deck es Escenario A (acotado al GIII+MV como portador principal del RTB de máxima capacidad). La discrepancia está resuelta en el flujo del proyecto; se documenta aquí para que el abogado tenga contexto si pregunta por el alcance del nombre en la gama.

### Discrepancia 2 — Sensor de temperatura: NTC (GSM-MB/RB) vs PTC100 (GIII+/GIII+MV)

El GSM-MB/RB usa sensor NTC para protección térmica (NODO-C). El GIII+/GIII+MV usa sensor PTC100 (platino) — tecnología de sensor diferente. El análisis competitivo identifica esta discrepancia como un elemento que afecta el alcance del claim de "NTC" en el NODO-C. La evaluación interna resuelve el punto: el NODO-C (Thermo-Safe) describe el NTC del GSM-MB/RB exclusivamente. El GIII+/GIII+MV no forma parte del NODO-C. Para el abogado: el claim de protección NTC en NODO-C aplica al GSM-MB/RB/RE. El GIII+/GIII+MV tiene protección de temperatura con PTC100, que es un mecanismo distinto que puede ser relevante para otros nodos en revisiones futuras pero no forma parte de este portafolio de naming.

### Pendiente técnico — Claims de diferenciación competitiva no verificados en fuente primaria

El análisis competitivo identifica en su matriz de diferenciación un subconjunto de hallazgos sobre la competencia marcados como inferencias de conocimiento técnico general (no verificados contra datasheet primario en la sesión de investigación). Los de mayor impacto en el posicionamiento de este Anexo son:
...
No se identificaron discrepancias entre el inventario técnico y el análisis competitivo, ni entre la evaluación legal interna y los datos técnicos del inventario. Las observaciones legales son consistentes con los RTBs documentados.
...
`C:\Raul\03-projects\genteca\portfolio-naming-ip-2026\06-three-pack\01-Anexo-Tecnico_v2.md`
...
2. **Decisión interna sobre alcance de curva inversa:** el caveat estructural se colocó una vez en la Sección 1 — Introducción, con el texto literal correspondiente. No se repite en los nodos individuales. La mención en NODO-B y NODO-D del alcance universal ("universal de facto") remite explícitamente a ese caveat de Sección 1.

3. **Discrepancias de inventario:** dos discrepancias identificadas (ForensLog 100 vs 20 fallas; NTC vs PTC100) están documentadas en Sección 5 con su resolución. No quedan discrepancias abiertas sin resolución en este documento.
...
5. **NODO-H GIO ecosystem:** el nombre anchor no existe (nodo nuevo sin precedente en el portafolio previo). La descripción técnica del nodo y la observación legal están orientadas a facilitar la evaluación de los candidatos de la familia GIO-* preparados internamente.
...
### Resumen de observaciones legales integradas

Todos los caveats literales de los 8 nodos están integrados en este documento:

- Caveat de alcance universal (curva inversa): Sección 1, caveat estructural único
- ThermalCurve rechazado (NODO-D): sección de observaciones legales + notas implícitas en diferenciación competitiva
- Thermo-Safe Caso A (NODO-C): observación literal integrada
- ForensLog Escenario A (NODO-E): observación literal integrada
- FlickerGuard contingencia OMPI (NODO-B): observación literal integrada
- TripleLock contingencia sectorial (NODO-F): observación literal integrada
- TaskMemory decisión diferenciada por jurisdicción (NODO-G): observación literal integrada
- Ecosystem GIO sin términos de protocolo estándar (NODO-H): observación literal integrada

### Items abiertos a I+D y al abogado
...
2. Confirmación denominación formal curva inversa V-t en documentos internos
...
*Versión 2.0 — Compilado por el equipo de propiedad intelectual de Genteca, 2026-05-13.*
