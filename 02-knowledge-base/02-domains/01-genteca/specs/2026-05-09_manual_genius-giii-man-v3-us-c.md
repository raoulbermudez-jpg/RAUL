```markdown
---
title: "GIII+ Electronic Total Motor Protection Relay"
type: Technical
source: "GIII-MAN-V3-US_c.pdf"
product_line: "Genius"
...
Once you have assembled CT Box on GIII (See item 3.1) follow these mounting instructions:
...
The menu includes adjustments for:
- DAY
- MONTH
- PR for Phase Reversal
...
When is chosen to protect against the charge loss through current monitoring, it is recommended a value by defect of 80% of the current at maximum charge (FLA) specified by the manufacturer.
...
- When is chosen to protect against charge loss through monitoring the power factor (PF), the GIII comes adjusted from factory with 0.5. (Although the protection can be adjusted in a range of 0.1 to 0.9 FP, it is recommended the adjustment above 0.3 FP).

## 11. GIII+ Connection's Diagram with External Current Transformers
...
a) The models identified as GIII+208000 and GIII+480000, are used exclusively with externals CT. These models are to protect motors up to 660A nominal current.

b) The user must specify the nominal motor current. With this information, will select the rank of current according the attached table, in which you will find the relation "/5" that require to select the CT to install.

c) The user must set up the GIII+ with external CT adding the instructions of adjustments contained in this application note. (All the others functions and protections will remain as specified in the installation's instruction GIII+).

d) The calibration of the GIII+ remain warrantee, as long as the external CT are commercial of class 1, secondary 5A.

### External CT Selection Table

| Rank of nominal current (Min-Max A) | Current transformers Relation "/5" |
|-----|-----|
| 150-200 | 600 |
| 190-250 | 750 |
| 200-260 | 800 |
...
**Example:** if a motor consume a nominal current of 350 amperes, the external toroids to select will be a value of 1200/5.

## 11.1 Adjustment of the External CT and Nominal Current in the Current Protection Menu

a) Press Up & Down Combination since the Operation screen to access voltage and current parameters.

g) Press Up & Down combination to get to NOMINAL I parameter (e.g., 300A).
```