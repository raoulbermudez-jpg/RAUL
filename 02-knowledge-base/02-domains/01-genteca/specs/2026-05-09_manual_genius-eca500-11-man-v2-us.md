...
type: Technical
source: "ECA500-11_MAN_V2_US.pdf"
product_line: "Genius"
...
**CAUTION:** Each set of ECA500-11 and detachable CT box is calibrated and have the same model's number. Before assembling the CT box verify that its model's number is the same as the ECA500-11 one. Failure to comply may result in measurement error and a loss of protection.
...
a) Place ECA500-11 at inclined position and its back side must be placed toward the upper edge of the DIN Rail and then push down ECA500-11 relay until it CLICKS in on the rail.

### 4.2 Installation with CT Box

Once you have assembled CT Box on ECA500-11, follow these mounting instructions:

a) Place ECA500-11 at inclined position so that its back side (CT Box) must be placed toward the upper edge of the DIN Rail and then push down ECA500-11 relay until it CLICKS in on the rail.

### 5.1 Panel Mounting without CT Box
...
b) Place ECA500-11 over a flat surface panel and mount it using (2) #8 x 1/2" screws. Make four (4) holes (5/32") on panel surface before installing. Refer Section 6.2 for dimensions.

### 5.2 Panel Mounting with CT Box

b) Place ECA500-11 with detachable box over a flat surface panel and mount it using (2) #8 x 1/2" screws. Make four (4) holes (5/32") on panel surface before installing. Refer Section 6.1 for dimensions.

## Wiring Diagram and Recommendations

### Wiring Recommendations

- Avoid over tightening M2.5 screw terminals during wiring of connections. Torque max: 4.5 Ib-in (5.18 kgf-cm).
...
- Connect L1L2L3 terminal for Voltage Input in parallel connection before line starter circuit through Contactor (as shown in Diagram Connection).

## Operation and Features

ECA500-11 constantly supervises current and voltage values; when any harmful condition occurs, the output connection is deactivated until the fault disappears and power line conditions returns to an acceptable level. Specific timing such as Start Up Delay and Fault Detection Delay are incorporated to prevent nuisance tripping due to rapid power fluctuations.

ECA500-11 provides LCD Display to indicate the output status (voltage, current, unbalance, frequency and load status) and has four (4) push buttons (ON/Off, Up, Down and Select) for electrical parameter adjustment such as Voltage, Current, Frequency, Fault Detection Delay and others. A Communication Port with MODBUS RTU protocol is included in ECA500-11.

UL Recognized SPD Type 2

## Setup Menu & Navigation

Press both push buttons at the same time in order to access the Main Menu (Screen #7). If access to screen is locked, you must enter the password required.
...
Manufacturers usually offer motors with limited range of operation.